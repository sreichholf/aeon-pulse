...
Standard enemy GLB conversions should not increase the previous per-enemy visible render-unit budget unless the exception is intentional, carefully justified, and sparse. Runtime-local presentation effects count toward that budget just like prepared model buckets. When a conversion replaces an existing procedural enemy, the old visible mesh count is the default budget target; for example, a three-mesh procedural enemy should normally remain at roughly three runtime render units after conversion.
