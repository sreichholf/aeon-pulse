...
- Adding a new enemy or boss to the viewer requires declaring its viewer projectile keys in `EntityCatalog`. That same catalog entry also declares model readiness and any presentation offsets, so the Tactical Database adapter imports no entity classes.
