...
## Amendment — Difficulty Modes Preserve The Campaign Curve

**Date:** 2026-06-16

ADR 0001 now defines the campaign as a chapter-shaped progression curve rather than a flat punishment ladder. Difficulty modes should preserve that curve instead of replacing it with separate progression rules.

### 1. Difficulty changes survivability first, not campaign growth targets

Rookie, Pilot, and Ace primarily change error tolerance:

- **Rookie** gives the largest learning buffer.
- **Pilot** gives a limited safety net while preserving the normal punishment loop after shield break.
- **Ace** remains the strict baseline.

They should not imply three unrelated weapon-progression models. The same chapter identity should remain recognizable in all modes.

### 2. Rookie should create learning space, not erase progression readability

Rookie intentionally removes weapon-tier loss on hits, but that is a survivability concession, not a license to flatten the rest of the campaign curve.

- Chapter 1 should still feel like a ramp.
- Mid-campaign should still feel stronger than the opening.
- Powerups should still matter even when the player is protected by shield rules.

If Rookie becomes the only mode where progression feels rewarding, the underlying campaign balance is wrong.

### 3. Pilot is the reference check for “fair but still demanding”

When validating progression changes, Pilot should be treated as the most important sanity-check mode:

- it preserves shielded learning moments
- it still exposes the player to tier loss
- it reveals whether recovery after mistakes is genuinely viable

If Pilot cannot recover cleanly from ordinary mid-run errors, the campaign is still too punishing even if Rookie feels acceptable.

## Consequences Of This Amendment

- Difficulty modes remain readable as survivability variants layered over one campaign.
- Balance discussions can separate “is the chapter curve right?” from “is the punishment too harsh?”
- Future tuning should avoid solving campaign-progression issues only through Rookie/Pilot shield generosity.
