...
describe('Terrain Classes Wall Queries', () => {
  ...
  describe('Terrain (Chapter 1/2)', () => {
    it('delegates wall interpolation to WallInterpolator and getCollisionWallsAt matches getWallsAt', () => {
      const scene = createMockScene();
      const terrain = new Terrain(scene, points);

      const walls = terrain.getWallsAt(250);
      expect(walls.top).toBe(125);
      expect(walls.bottom).toBe(-110);
      expect(terrain.getCollisionWallsAt(250)).toEqual(walls);
    });
  });
  ...
  describe('Terrain4 (Chapter 4 Volcanic)', () => {
    ...
    it('clamps getCollisionWallsAt to be more restrictive when column/crystal obstacles overlap', () => {
      const scene = createMockScene();
      const terrain = new Terrain4(scene, points);

      const baseWalls = terrain.getWallsAt(250);
      const actualWalls = terrain.getCollisionWallsAt(250);
