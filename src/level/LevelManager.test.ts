...
describe('LevelManager', () => {
  it('starts at scrollX = 0 and updates the background baseSpeed on initialization', () => {
    ...
    const content = resolveLevelContent(getCampaignLevel('1-1')); // scrollSpeed: 100
    const manager = new LevelManager(host, content);
  ...
  it('triggers boss spawning on finale level once waves are cleared and scrollX >= bossAt', () => {
    const host = createMockHost();
    const content = resolveLevelContent(getCampaignLevel('1-5')); // isFinale: true, archetype 1, bossAt: 7300
    const manager = new LevelManager(host, content);
  ...
  it('triggers level completion on non-finale level only when waves are cleared and clear gate is open', () => {
    ...
    const content = resolveLevelContent(getCampaignLevel('1-1')); // isFinale: false, scrollSpeed: 100, last wave: 7800
    const manager = new LevelManager(host, content);
  ...
  it('does not spawn ambient popcorn on levels with ambient pressure disabled', () => {
    const host = createMockHost();
    ...
    expect(host.hasEnemyNear).not.toHaveBeenCalled();
    expect(host.spawnEnemy).not.toHaveBeenCalled();
  });

  it('checks for nearby enemies when spawning ambient popcorn on enabled levels', () => {
    const host = createMockHost();
    const content = resolveLevelContent(getCampaignLevel('1-2'));
    const manager = new LevelManager(host, content);

    // Light ambient pressure starts later than standard pressure.
    manager.update(2.7);
