...
import { BulletType, EnemyType, UserDataKey, type GetPositionFn, type IAudio, type IScene, type ProjectileFactoryFn } from '../types.ts';
import { mergeGeometries } from 'three/examples/jsm/utils/BufferGeometryUtils.js';
import { ensureNonIndexed, addVertexColor, ProceduralResourceCache, setMaterialBucket } from '../utils/ProceduralToolkit.ts';
...
interface EnemyTurretResources {
  geometries: {
    axle: THREE.BufferGeometry;
    ring: THREE.BufferGeometry;
    gear: THREE.BufferGeometry;
    sleeve: THREE.BufferGeometry;
    vent: THREE.BufferGeometry;
    coil: THREE.BufferGeometry;
    muzzle: THREE.BufferGeometry;
    cannonFlash: THREE.BufferGeometry;
  };
  materials: {
    base: THREE.MeshPhongMaterial;
    gear: THREE.MeshPhongMaterial;
    sleeve: THREE.MeshPhongMaterial;
    ventTemplate: THREE.MeshPhongMaterial;
    coilTemplate: THREE.MeshPhongMaterial;
    muzzleTemplate: THREE.MeshPhongMaterial;
    ringTemplate: THREE.MeshPhongMaterial;
  };
}

export class EnemyTurret extends Enemy {
  private static _cache = new ProceduralResourceCache<EnemyTurretResources>();

  static initSharedResources(): void {
    EnemyTurret._cache.init(() => {
    ...
    setMaterialBucket(baseMat, 'body');
    ...
    setMaterialBucket(gearMat, 'body');
    ...
    setMaterialBucket(sleeveMat, 'body');

    const ventTemplate = new THREE.MeshPhongMaterial({
      color: 0xff4400,      // Recessed glowing warning-orange vents
      emissive: 0x440a00,
      shininess: 50,
      specular: 0xff9977,
    });
    setMaterialBucket(ventTemplate, 'glow');

    const coilTemplate = new THREE.MeshPhongMaterial({
      color: 0xff6600,      // Bright glowing copper coils
      emissive: 0x3d0f00,
      shininess: 80,
      specular: 0xffffff,
    });
    setMaterialBucket(coilTemplate, 'glow');

    const muzzleTemplate = new THREE.MeshPhongMaterial({
      color: 0x33e6ff,      // Vibrant neon-cyan railgun muzzle emitter
      emissive: 0x004455,
      shininess: 80,
      specular: 0xffffff,
    });
    setMaterialBucket(muzzleTemplate, 'glow');

    const ringTemplate = new THREE.MeshPhongMaterial({
      color: 0xff8800,      // Saturated glowing energy containment rings
      emissive: 0x441100,
      shininess: 80,
      specular: 0xffffff,
    });
    setMaterialBucket(ringTemplate, 'glow');
    ...
    if (!mergedRingGeo) throw new Error('EnemyTurret: failed to merge ring geometry');
    mergedRingGeo.computeVertexNormals();
    mergedRingGeo.computeBoundingBox();
    mergedRingGeo.computeBoundingSphere();
    ...
    spokeHCloned.rotateZ(Math.PI / 2);
    ...
    if (!mergedGearGeo) throw new Error('EnemyTurret: failed to merge gear geometry');
    mergedGearGeo.computeVertexNormals();
    mergedGearGeo.computeBoundingBox();
    mergedGearGeo.computeBoundingSphere();
    ...
    if (!mergedSleeveGeo) throw new Error('EnemyTurret: failed to merge sleeve geometry');
    mergedSleeveGeo.computeVertexNormals();
    mergedSleeveGeo.computeBoundingBox();
    mergedSleeveGeo.computeBoundingSphere();
    ...
    if (!mergedVentGeo) throw new Error('EnemyTurret: failed to merge vent geometry');
    mergedVentGeo.computeVertexNormals();
    mergedVentGeo.computeBoundingBox();
    mergedVentGeo.computeBoundingSphere();
    ...
    // Merge Cannon Flash Geometry (Sleeve + Vent + 4 Coils + Muzzle)
    const cannonFlashGeos: THREE.BufferGeometry[] = [];
    const sleeveClean = mergedSleeveGeo.clone();
    if (sleeveClean.hasAttribute('color')) sleeveClean.deleteAttribute('color');
    cannonFlashGeos.push(sleeveClean);
    cannonFlashGeos.push(mergedVentGeo.clone());

    for (let i = 0; i < 4; i++) {
      const cCloned = ensureNonIndexed(coilGeo);
      cCloned.translate(-6 - i * 6, 0, 0);
      cannonFlashGeos.push(cCloned);
    }
    const mCloned = ensureNonIndexed(muzzleGeo);
    mCloned.translate(-29, 0, 0);
    cannonFlashGeos.push(mCloned);

    const mergedCannonFlashGeo = mergeGeometries(cannonFlashGeos);
    if (!mergedCannonFlashGeo) throw new Error('EnemyTurret: failed to merge cannon flash geometry');
    mergedCannonFlashGeo.computeVertexNormals();
    mergedCannonFlashGeo.computeBoundingBox();
    mergedCannonFlashGeo.computeBoundingSphere();

    cannonFlashGeos.forEach(g => g.dispose());
    ...
    return {
      geometries: {
        ...
        ring: mergedRingGeo,
        gear: mergedGearGeo,
        sleeve: mergedSleeveGeo,
        vent: mergedVentGeo,
        coil: coilGeo,
        muzzle: muzzleGeo,
        cannonFlash: mergedCannonFlashGeo,
      },
      materials,
    };
  });
  ...
  private _headOverlay: THREE.Mesh | null = null;
  private _cannonOverlay: THREE.Mesh | null = null;
  ...
  constructor(
    ...
    const mats = EnemyTurret._cache.resources.materials;

    // Clone mutating/animating materials per-instance
    this._ventMat = mats.ventTemplate.clone();
    this._coilMat = mats.coilTemplate.clone();
    this._muzzleMat = mats.muzzleTemplate.clone();
    this._ringMat = mats.ringTemplate.clone();
  ...
  private _build3DModel(): THREE.Group {
    ...
    const geos = EnemyTurret._cache.resources.geometries;
    const mats = EnemyTurret._cache.resources.materials;

    // 2. Static Background Mount (Z-axis axle cylinder + dual containment toruses)
    const axle = new THREE.Mesh(geos.axle, mats.base);
    group.add(axle);

    const ringMesh = new THREE.Mesh(geos.ring, this._ringMat!);
    group.add(ringMesh);
    ...
    // 3A. Huge Rotating Gear Body
    const gearMesh = new THREE.Mesh(geos.gear, mats.gear);
    ...
    const sleeveMesh = new THREE.Mesh(geos.sleeve, mats.sleeve);
    this._cannonGroup.add(sleeveMesh);

    const ventMesh = new THREE.Mesh(geos.vent, this._ventMat!);
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const coil = new THREE.Mesh<THREE.BufferGeometry, THREE.MeshPhongMaterial>(geos.coil, cMat);
      coil.position.set(-6 - i * 6, 0, 0);
      this._cannonGroup.add(coil);
      this._coils.push(coil);
    }

    // Octagonal muzzle emitter
    const muzzle = new THREE.Mesh(geos.muzzle, this._muzzleMat!);
    ...
    // 5. Flash overlays for moving components
    this._headOverlay = new THREE.Mesh(geos.gear, DEFAULT_FLASH_MATERIAL);
    this._headOverlay.visible = false;
    this._headOverlay.renderOrder = 20;
    this._headGroup.add(this._headOverlay);

    this._cannonOverlay = new THREE.Mesh(geos.cannonFlash, DEFAULT_FLASH_MATERIAL);
    this._cannonOverlay.visible = false;
    this._cannonOverlay.renderOrder = 20;
    this._cannonGroup.add(this._cannonOverlay);

    group.userData.isInstanced = true;
    group.userData[UserDataKey.ENEMY_TYPE] = EnemyType.TURRET;
    return group;
  }
  ...
  override _flash(): void {
    if (this._headOverlay) this._headOverlay.visible = true;
    if (this._cannonOverlay) this._cannonOverlay.visible = true;
    this._hitFlashTimer = 0.08;
  }

  override _restoreFlash(): void {
    if (this._headOverlay) this._headOverlay.visible = false;
    if (this._cannonOverlay) this._cannonOverlay.visible = false;
  }
  ...
  override destroy(): void {
    ...
    if (this._ventMat) {
      this._ventMat.dispose();
      this._ventMat = null;
    }
    if (this._coilMat) {
      this._coilMat.dispose();
      this._coilMat = null;
    }
    if (this._muzzleMat) {
      this._muzzleMat.dispose();
      this._muzzleMat = null;
    }
    if (this._ringMat) {
    ...
    if (this._coils) {
      for (const coil of this._coils) {
        coil.material.dispose();
      }
      this._coils = [];
    }

    this._headGroup = null;
    this._cannonGroup = null;
    this._muzzle = null;
    this._headOverlay = null;
    this._cannonOverlay = null;
  }
}
