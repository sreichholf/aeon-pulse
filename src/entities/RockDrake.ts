...
import { EnemyType, ProjectileSourceKey, UserDataKey, type GetPositionFn, type IBullet, type IScene, type ProjectileFactoryFn } from '../types.ts';
...
export class RockDrake extends Enemy {
  private static _cache = new ProceduralResourceCache<RockDrakeResources>();

  static initSharedResources(): void {
    RockDrake._cache.init(() => {
      // 1. Materials
      ...
      setMaterialBucket(clawMat, 'body');

      const eyeMat = new THREE.MeshBasicMaterial({
        color: VOLCANIC_COLORS.LAVA_TIP,
      });
      setMaterialBucket(eyeMat, 'glow');
  ...
  constructor(scene: IScene, sprites: Record<string, THREE.Texture>, x: number, y: number, getPlayerPos: GetPositionFn, projectileFactory: ProjectileFactoryFn) {
    ...
    const { materials: mats } = RockDrake._cache.resources;
  ...
  private _build3DModel(): THREE.Group {
    ...
    const { geometries: geos, materials: mats } = RockDrake._cache.resources;
    ...
    group.userData[UserDataKey.ENEMY_TYPE] = EnemyType.ROCK_DRAKE;
    return group;
  }
