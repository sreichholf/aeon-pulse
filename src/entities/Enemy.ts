...
import { ProjectileSourceKey, type EnemyPresentationContext, type HitResult, type GetPositionFn, type TerrainBounds, type IBullet, type EntityMetadata, type IEnemy, type IScene, type ProjectileFactoryFn } from '../types.ts';
...
export class Enemy extends Entity implements IEnemy {
  ...
  protected _presentationContext: EnemyPresentationContext;
  ...
  constructor(
    ...
    presentationContext: EnemyPresentationContext = 'gameplay',
  ) {
    super();
    this._scene   = scene;
  ...
  setPosition(x: number, y: number): void {
    if (this._mesh) this._mesh.position.set(x, y, 0);
  }
