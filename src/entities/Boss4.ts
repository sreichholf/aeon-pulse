...
import { ensureNonIndexed, addVertexColor, ProceduralResourceCache, VOLCANIC_MATERIAL_PARAMS } from '../utils/ProceduralToolkit.ts';
...
interface Boss4Resources {
  materials: {
    rock: THREE.MeshPhongMaterial;
    facet: THREE.MeshPhongMaterial;
    plate: THREE.MeshPhongMaterial;
    leg: THREE.MeshPhongMaterial;
  };
}

export class Boss4 extends BossBase {
  private static _cache = new ProceduralResourceCache<Boss4Resources>();

  static initSharedResources(): void {
    Boss4._cache.init(() => {
      return {
        materials: {
          rock: new THREE.MeshPhongMaterial({ ...VOLCANIC_MATERIAL_PARAMS.rock, flatShading: true, vertexColors: true }),
          facet: new THREE.MeshPhongMaterial({ ...VOLCANIC_MATERIAL_PARAMS.armor, flatShading: true, vertexColors: true }),
          plate: new THREE.MeshPhongMaterial({ color: 0x685a50, emissive: 0x2b1b11, specular: 0x473b32, shininess: 15, flatShading: true, vertexColors: true }),
          leg: new THREE.MeshPhongMaterial({ color: 0x7a5f4c, emissive: 0x24180f, specular: 0x544031, shininess: 15, flatShading: true, vertexColors: true }),
        },
      };
    });
  }
  ...
  constructor({ scene, sprites, getPlayerPos, onDeath, audio, spawnEnemy, projectileFactory, presentationContext }: BossConstructorParams) {
    super(scene, sprites, getPlayerPos, onDeath, audio, STOP_X, ENTRY_SPEED, HP_PLATE * 2 + HP_CORE, DISPLAY_W, DISPLAY_H, projectileFactory, presentationContext);
    ...
    if (this._mesh && this._presentationContext === 'gameplay') {
  ...
  protected override _initPresentationPose(): void {
    if (this._mesh) {
      this._mesh.position.set(STOP_X, 130, 0);
    }
  }
  ...
  private _buildLeg(xOffset: number, zOffset: number, isFront: boolean, isLeftLeg: boolean): THREE.Group {
    ...
    const legMat = Boss4._cache.resources.materials.leg;
  ...
  protected _build3DModel(): THREE.Object3D {
    ...
    const mats = Boss4._cache.resources.materials;
    ...
    segmentConfigs.forEach((cfg, idx) => {
      ...
      if (cfg.type === 'head') {
        ...
        const rearMesh = new THREE.Mesh(rearGeo, mats.rock);
        ...
        const h1 = new THREE.Mesh(h1Geo, mats.facet);
        ...
        const h2 = new THREE.Mesh(h2Geo, mats.facet);
        ...
        const h3 = new THREE.Mesh(h3Geo, mats.facet);
        ...
        const midMesh = new THREE.Mesh(midGeo, mats.rock);
        this._headSeg2.add(midMesh);

        const jawSpineR = new THREE.Mesh(new THREE.ConeGeometry(2, 10, 4), mats.facet);
        jawSpineR.position.set(0, -3, 5);
        jawSpineR.rotation.set(Math.PI / 3.5, 0, -Math.PI / 6);
        this._headSeg2.add(jawSpineR);

        const jawSpineL = new THREE.Mesh(new THREE.ConeGeometry(2, 10, 4), mats.facet);
        ...
        const frontMesh = new THREE.Mesh(frontGeo, mats.rock);
        ...
        collapseStaticMeshChildren(this._headSeg1, mats.rock, animatedHeadMaterials);
        collapseStaticMeshChildren(this._headSeg2, mats.rock, animatedHeadMaterials);
        collapseStaticMeshChildren(this._headSeg3, mats.rock, animatedHeadMaterials);
        ...
        segGroup.add(mergedColoredMesh(segmentGeos, mats.rock));
        bodyGeo.dispose();
        scaleGeo.dispose();
        ...
        segGroup.add(mergedColoredMesh(tailGeos, mats.rock));
        tailGeo.dispose();
        spikeGeo.dispose();
      }
    ...
    this._plateLMesh.add(mergedColoredMesh(plateLGeos, mats.plate));
    ...
    this._plateRMesh.add(mergedColoredMesh(plateRGeos, mats.plate));
