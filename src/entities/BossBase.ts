...
import type { BossPresentationContext, GetPositionFn, IBullet, EntityMetadata, IAudio, IScene, IBoss, HitZone, ICollidable, PlayfieldBounds, ProjectileFactoryFn } from '../types.ts';
...
export abstract class BossBase extends Entity implements IBoss {
  ...
  protected _presentationContext: BossPresentationContext;
  ...
  protected _initPresentationPose(): void {}
  ...
  constructor(
    ...
    presentationContext: BossPresentationContext = 'gameplay',
  ) {
    super();
    this._scene        = scene;
  ...
  protected _init(): void {
    ...
    if (this._presentationContext === 'viewer') {
      this._mesh.position.set(this._stopX, 0, 0);
      this._entered = true;
      this._initPresentationPose();
    } else {
