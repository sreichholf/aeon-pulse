...
import { type IScene, PowerUpType } from '../types.ts';
...
export class PowerUp {
  ...
  constructor(scene: IScene, x: number, y: number, type: PowerUpType = PowerUpType.WEAPON) {
    this._scene = scene;
    this._baseY = y;
    this._time  = 0;
    this.type = type;
    ...
    const coreMat = new THREE.MeshPhongMaterial({
      color: isBomb ? 0xffaa00 : 0xff00ee,
      emissive: isBomb ? 0x442200 : 0x4a0044,
      specular: isBomb ? 0xffffaa : 0xffaaff,
      shininess: 120,
      transparent: true,
      opacity: 0.88,
    ...
    const ringMat = new THREE.MeshPhongMaterial({
      color: isBomb ? 0xffdd00 : 0x00ffcc,
      emissive: isBomb ? 0x443300 : 0x004433,
      specular: 0xffffff,
      shininess: 100,
    });
    ...
    markRenderCategory(this._mesh, RenderCategory.EFFECT, 'powerup');
