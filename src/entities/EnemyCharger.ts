...
import type { EnemyPresentationContext, GetPositionFn, IAudio, IScene, ProjectileFactoryFn } from '../types.ts';
...
export class EnemyCharger extends Enemy {
  ...
  constructor(
    ...
    options: EnemyChargerOptions = {},
  ) {
    super(scene, sprites, null, 0, 0, HW, HH, x, y, projectileFactory, options.presentationContext);
    this._hp             = 1;
    this.score           = 250;
  ...
  private _applyViewerPose(): void {
    this._topWingGroup!.rotation.z = -0.05;
    this._bottomWingGroup!.rotation.z = 0.05;
    this._shipGroup!.position.set(0, 0, 0);
    ...
    this._laserMesh!.scale.set(0, 0, 0);
