...
interface AudioManagerOptions {
  startTestAudioSuppressed?: boolean;
}
...
export class AudioManager implements IAudio {
  ...
  private _isBrowserTestAudioRun: boolean;
  private _isTestAudioSuppressed: boolean;
  private _deferredCue: MusicCue | null;
  private _runtimeListeners: Set<(state: AudioRuntimeState) => void>;
  ...
  constructor(options: AudioManagerOptions = {}) {
    ...
    this._isBrowserTestAudioRun = options.startTestAudioSuppressed ?? false;
    this._isTestAudioSuppressed = options.startTestAudioSuppressed ?? false;
    this._deferredCue = null;
  ...
  play(sound: string, ...args: unknown[]): void {
    if (this._isTestAudioSuppressed) return;
  ...
  get isBrowserTestAudioRun(): boolean {
    return this._isBrowserTestAudioRun;
  }

  get isTestAudioSuppressed(): boolean {
    return this._isTestAudioSuppressed;
  }

  onRuntimeStateChange(listener: (state: AudioRuntimeState) => void): () => void {
    this._runtimeListeners.add(listener);
    listener(this._getRuntimeState());
    return () => {
      this._runtimeListeners.delete(listener);
    };
  }

  toggleTestAudioForRun(): void {
    if (!this._isBrowserTestAudioRun) return;
    this.setTestAudioSuppressed(!this._isTestAudioSuppressed);
  }

  setTestAudioSuppressed(suppressed: boolean): void {
    if (!this._isBrowserTestAudioRun || this._isTestAudioSuppressed === suppressed) return;

    this._isTestAudioSuppressed = suppressed;
    if (suppressed) {
      this.stopChargeHum();
    ...
    if (this._deferredCue !== null) {
      try {
        this._sequencer.playCue(this._deferredCue, resolveThemeDefinition(this._deferredCue));
      } catch (_) {}
    }
    this._emitRuntimeState();
  }
  ...
  private _getRuntimeState(): AudioRuntimeState {
    return {
      volume: this._volume,
      isBrowserTestAudioRun: this._isBrowserTestAudioRun,
      isTestAudioSuppressed: this._isTestAudioSuppressed,
    };
  }

  private _emitRuntimeState(): void {
