...
import { ensureNonIndexed, addVertexColor, setMaterialBucket, getMaterialBucket } from './ProceduralToolkit.ts';
...
describe('ProceduralToolkit', () => {
  ...
  describe('material bucket helpers', () => {
    it('round-trips bucket label through material userData', () => {
      const material = new THREE.MeshBasicMaterial();
      expect(getMaterialBucket(material)).toBe('body');
      setMaterialBucket(material, 'glass');
      expect(getMaterialBucket(material)).toBe('glass');
      setMaterialBucket(material, 'glow');
      expect(getMaterialBucket(material)).toBe('glow');
    });

    it('defaults to body when bucket is not set', () => {
      const material = new THREE.MeshBasicMaterial();
      expect(getMaterialBucket(material)).toBe('body');
    });
  });
