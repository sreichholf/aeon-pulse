...
export class TitleScreen {
  ...
  constructor(showAdvancedOptions = ENABLE_ADVANCED_TITLE_OPTIONS) {
    ...
    this.el.classList.toggle('title-with-dev', this._showAdvancedOptions);
  ...
  private _build(): void {
    const s = this.el;
    const titleTop = this._el('div', 'title-top');
    titleTop.appendChild(this._el('div', 'title-logo', 'AEON PULSE'));
    titleTop.appendChild(this._el('div', 'title-kicker', 'ARCADE STRIKE RUN'));
    s.appendChild(titleTop);
    ...
    const titleMain = this._el('div', 'title-main');
    ...
    modeSelection.appendChild(this._el('span', 'title-mode-hint', 'TAB TO CHANGE'));
    titleMain.appendChild(modeSelection);

    const utilityRow = this._el('div', 'title-utility-row');
    utilityRow.appendChild(this._el('div', 'title-help-copy', 'ROOKIE EASES YOU IN. PILOT AND ACE EXPECT CLEAN DODGING.'));
    utilityRow.appendChild(this._el('div', 'title-viewer-hint', 'V DATABASE  |  M MUSIC'));
    titleMain.appendChild(utilityRow);

    s.appendChild(titleMain);

    if (this._showAdvancedOptions) {
      const devPanel = this._el('div', 'title-dev-panel');
      devPanel.appendChild(this._el('div', 'title-dev-label', 'DEVELOPER OPTIONS'));
      ...
      devPanel.appendChild(selectors);
      s.appendChild(devPanel);
    }

    const scoresSection = this._el('div', 'title-scores-section');
    this._titleScoresHeader = this._el('div', 'title-scores-header', 'HIGH SCORES');
    this._titleScoresList = this._el('div', 'title-scores-list');
    scoresSection.appendChild(this._titleScoresHeader);
    scoresSection.appendChild(this._titleScoresList);
    s.appendChild(scoresSection);
  }
