...
export class HUDScreen {
  ...
  private _bombsBox!: HTMLElement;
  private _bombIcons!: HTMLElement[];
  ...
  updateHUD({ score = 0, hiScore = 0, lives = MAX_LIVES, bombs = 2, maxBombs = 2, chargeLevel = 0, weaponTier = 1,
    ...
    this._bombIcons.forEach((icon, i) => {
      icon.style.display = i >= maxBombs ? 'none' : 'block';
      icon.classList.toggle('empty', i >= bombs);
    });
  ...
  private _build(): void {
    ...
    this._bombsBox = this._el('div', 'hud-bombs');
    this._bombsBox.innerHTML = '<span class="hud-label">BOMBS</span>';
    const bombsIconsBox = this._el('div', 'hud-bombs-icons');
    this._bombIcons = [];
    for (let i = 0; i < 5; i++) {
      const icon = this._el('div', 'hud-bomb');
      this._bombIcons.push(icon);
      bombsIconsBox.appendChild(icon);
    }
    this._bombsBox.appendChild(bombsIconsBox);
    rightContainer.appendChild(this._bombsBox);
