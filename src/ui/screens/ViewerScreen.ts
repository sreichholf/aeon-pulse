...
export class ViewerScreen {
  ...
  /** Background frames keyed by card index, used to drive the Signal Acquisition Reveal (ADR 0023). */
  private _bgCards: HTMLElement[] = [];
  ...
  show(page: number, entities: ViewerEntityData[]): void {
    ...
    for (const ent of entities) {
      ...
      // Background frame starts in the "acquiring" scan state (Signal Acquisition Reveal,
      // ADR 0023); revealViewerCard() materializes it once its model attaches.
      const bgCard = this._el('div', 'viewer-bg-card acquiring' + (isBoss ? ' boss-card' : ''));
      bgCard.style.left = `${left}px`;
      bgCard.style.top = `${top}px`;

      const scanClip = this._el('div', 'viewer-card-scan-clip');
      // The raster grid stays visible as a holographic backdrop even after the model loads.
      scanClip.appendChild(this._el('div', 'viewer-card-grid'));
      // The "acquiring" label fades out once the subject materializes.
      const acquire = this._el('div', 'viewer-card-acquire');
      acquire.appendChild(this._el('div', 'viewer-card-acquire-label', 'ACQUIRING SIGNAL'));
      scanClip.appendChild(acquire);
      scanClip.appendChild(this._el('div', 'viewer-card-reveal-line'));
      bgCard.appendChild(scanClip);

      this._viewerBgCardsContainer.appendChild(bgCard);
      this._bgCards.push(bgCard);
    }
  }

  /** Drop the acquiring scan state and play the one-shot materialize sweep for one card. */
  revealViewerCard(index: number): void {
    const bgCard = this._bgCards[index];
    if (!bgCard) return;
    bgCard.classList.remove('acquiring');
    bgCard.classList.add('revealing');
  }
