...
export class VolumeControl {
  ...
  private _unsubscribeRuntimeState: (() => void) | null;
  ...
  constructor(audio: AudioManager | null) {
    ...
    this._unsubscribeRuntimeState = null;
  ...
  private _build(): void {
    ...
    const indicator = this._el('span', 'test-audio-indicator');
    this._indicator = indicator;
    volCtrl.appendChild(indicator);
    ...
    btn.addEventListener('click', (e) => {
      ...
      if (this._audio.isBrowserTestAudioRun) {
        this._audio.toggleTestAudioForRun();
        return;
      }
      if (this._isMuted) {
    ...
    if (this._audio && typeof this._audio.onRuntimeStateChange === 'function') {
      this._unsubscribeRuntimeState = this._audio.onRuntimeStateChange((state) => {
        this._syncFromAudioState(state.volume, state.isBrowserTestAudioRun, state.isTestAudioSuppressed);
      });
    } else {
      this._syncFromAudioState(initialVol, false, false);
    }
  }

  destroy(): void {
    this._unsubscribeRuntimeState?.();
    this._unsubscribeRuntimeState = null;
  }

  private _syncFromAudioState(volume: number, isBrowserTestAudioRun: boolean, isTestAudioSuppressed: boolean): void {
    if (this._slider) {
      this._slider.value = String(Math.round(volume * 100));
    }
    if (this._tooltip) {
      this._tooltip.textContent = `${Math.round(volume * 100)}%`;
    }

    this._isMuted = volume === 0;
    if (volume > 0) {
      this._savedVolBeforeMute = volume;
    }
    this._updateVolumeIcon(isBrowserTestAudioRun && isTestAudioSuppressed ? 0 : volume);

    if (this._hint) {
      this._hint.textContent = isBrowserTestAudioRun ? 'M TO TOGGLE TEST AUDIO' : 'M TO TOGGLE MUSIC';
    }

    if (this._indicator) {
      if (isBrowserTestAudioRun) {
        this._indicator.textContent = isTestAudioSuppressed ? 'TEST AUDIO OFF' : 'TEST AUDIO ON';
        this._indicator.classList.toggle('suppressed', isTestAudioSuppressed);
        this._indicator.classList.toggle('enabled', !isTestAudioSuppressed);
      } else {
        this._indicator.textContent = '';
        this._indicator.classList.remove('suppressed', 'enabled');
      }
    }

    this.el.classList.toggle('test-audio-run', isBrowserTestAudioRun);
    this.el.classList.toggle('test-audio-suppressed', isBrowserTestAudioRun && isTestAudioSuppressed);
  }
