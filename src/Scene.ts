...
import { EnemyInstancer, type EnemyInstancerSnapshot } from './systems/EnemyInstancer.ts';
...
export class Scene {
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    markRenderCategory(this._flashMesh, RenderCategory.EFFECT, 'flashOverlay');
  ...
  render(dt: number = 0): void {
    ...
    measurePerfPhase('scene.instancedEnemies', () => {
      ...
      this._activeInstancedEnemies.forEach((enemy) => {
        if (enemy.position.x >= -halfW - margin && enemy.position.x <= halfW + margin) {
  ...
  getEnemyAttributionSnapshot(): EnemyInstancerSnapshot {
    return this._enemyInstancer.getAttributionSnapshot();
  }
