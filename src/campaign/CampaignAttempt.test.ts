...
describe('CampaignAttempt', () => {
  ...
  it('should allow updating level and weapon tier through the progression surface', () => {
  ...
  describe('advance', () => {
    it('should advance to the next level and preserve the weapon tier', () => {
      const level = getCampaignLevel('1-1');
    ...
    it('should throw an error when trying to advance past the final level', () => {
      ...
      expect(() => attempt.advance()).toThrowError('No next level to advance to.');
    });
  });
});
